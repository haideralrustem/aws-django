...
def validate_response(response):
    ...
    if response:
        if response.status_code and response.status_code == 200:
            if response.json()  and 'category_list' in response.json():
                
                if len(response.json()['category_list']) > 0:
                    for cat in response.json()['category_list']:
                        likely_topic += cat['label']
                        likely_topic += ', '
                    success = True
                else:
                    likely_topic = " category_list is empty "
            else:
                likely_topic = 'category_list not in resonse.json()'
                
        else:
            likely_topic = 'response status code not 200'
    else:
        likely_topic = 'no response object'

    
    return  likely_topic, success
